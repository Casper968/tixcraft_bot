...
def tixcraft_ticket_main_agree(driver, config_dict):
    ...
    for i in range(3):
        ...
        # is_finish_checkbox_click = check_checkbox_by_label(driver, By.CSS_SELECTOR, 'label[for="TicketForm_agree"]')
...
def check_checkbox_by_label(driver, by, query):
    show_debug_message = True       # debug.
    show_debug_message = False      # online

    label_element = None
    try:
        label_element = driver.find_element(by, query)
    except Exception as exc:
        if show_debug_message:
            print(exc)
        pass
